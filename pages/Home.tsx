...
const Home: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white font-sans overflow-x-hidden">
      ...
      {/* 🏛️ SECTION 1: ARTISTIC HERO CAROUSEL - RESPONSIVE FIX */}
      <section className="relative h-[85vh] md:h-[90vh] bg-comfort-dark overflow-hidden">
        {heroItems.map((post, idx) => (
          ...
          >
            <div className="absolute inset-0">
               ...
               />
               <div className="absolute inset-0 bg-gradient-to-r from-comfort-dark/95 via-comfort-dark/70 md:via-comfort-dark/60 to-comfort-dark/40 md:to-transparent"></div>
            </div>
            ...
            <div className="container relative z-20 mx-auto h-full flex items-center px-6 md:px-12">
              <div className="max-w-4xl animate-fade-in-up mt-12 md:mt-0">
                <div className="flex items-center space-x-4 mb-4 md:mb-6">
                   <span className="h-[2px] w-8 md:w-12 bg-comfort-gold"></span>
                   <span className="text-comfort-gold font-bold tracking-[0.2em] md:tracking-[0.3em] uppercase text-[10px] md:text-xs">{post.category}</span>
                </div>
                <h1 className="text-3xl sm:text-4xl md:text-7xl font-serif font-bold text-white leading-[1.2] md:leading-[1.1] mb-6 md:mb-8 text-balance">
                  {post.title}
                </h1>
                <p className="text-sm md:text-xl text-gray-300 mb-8 md:text-xl mb-10 font-light leading-relaxed max-w-2xl border-l-2 border-white/20 pl-4 md:pl-6 line-clamp-3 md:line-clamp-none">
                  {post.excerpt}
                </p>
                <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                  <Link to={`/blog/${post.id}`} className="bg-comfort-gold text-white px-8 md:px-10 py-3.5 md:py-4 font-bold uppercase tracking-widest hover:bg-white hover:text-comfort-dark transition-all duration-500 shadow-2xl text-center text-xs md:text-sm">
                    Découvrir l'histoire
                  </Link>
                  <Link to="/donate" className="border border-white/30 text-white px-8 md:px-10 py-3.5 md:py-4 font-bold uppercase tracking-widest hover:bg-white/10 transition-all backdrop-blur-sm text-center text-xs md:text-sm">
                    Soutenir l'action
                  </Link>
                </div>
              </div>
        ...
        {/* Carousel Nav Controls - Responsive Repositioning */}
        <div className="absolute bottom-6 right-6 md:bottom-12 md:right-12 z-30 flex items-center space-x-4 md:space-x-6">
           <button onClick={prevHero} className="text-white/50 hover:text-white transition-colors border border-white/20 p-2 md:p-4 rounded-full hover:border-comfort-gold">
           ...
           <button onClick={nextHero} className="text-white/50 hover:text-white transition-colors border border-white/20 p-2 md:p-4 rounded-full hover:border-comfort-gold">
        ...
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 md:gap-24 items-center">
            <div className="relative">
              <div className="absolute -top-6 -left-6 md:-top-12 md:-left-12 text-[10rem] md:text-[15rem] font-serif text-gray-50 opacity-10 select-none z-0">C</div>
              <p className="text-comfort-gold font-bold uppercase tracking-[0.3em] md:tracking-[0.4em] mb-4 relative z-10 text-xs md:text-sm">{t('about_section.tag')}</p>
            ...
            <div className="grid grid-cols-2 gap-4 md:gap-6 relative">
               <div className="space-y-4 md:space-y-6 pt-8 md:pt-12">
                  ...
                  <div className="p-6 md:p-8 bg-comfort-blue text-white rounded-sm">
                    <h4 className="text-2xl md:text-3xl font-serif font-bold mb-1 md:mb-2">240+</h4>
                    <p className="text-[10px] uppercase tracking-widest opacity-70">Programmes achevés</p>
                  </div>
               </div>
               <div className="space-y-4 md:space-y-6">
                  <div className="p-6 md:p-8 bg-comfort-gold text-white rounded-sm">
                    <h4 className="text-2xl md:text-3xl font-serif font-bold mb-1 md:mb-2">15k</h4>
                    <p className="text-[10px] uppercase tracking-widest opacity-70">Vies impactées</p>
                  </div>
                  <div className="aspect-[3/4] rounded-sm overflow-hidden shadow-2xl">
      ...
      <section className="py-20 md:py-32 bg-comfort-light border-y border-gray-100">
        <div className="container mx-auto px-6">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-0 border border-gray-100">
              {DOMAINS.map((domain) => (
                <div key={domain.id} className="group p-8 md:p-12 bg-white border-r border-b border-gray-100 hover:bg-comfort-blue transition-all duration-700 relative overflow-hidden">
                   ...
                   <div className="mb-8 md:mb-10 transform group-hover:-translate-y-2 transition-transform duration-500">
                      {getIcon(domain.id)}
                   </div>
                   <h3 className="text-base md:text-lg font-bold text-comfort-dark group-hover:text-white mb-4 uppercase tracking-widest">{t(`domains.${domain.id}.title`)}</h3>
                   <p className="text-xs md:text-sm text-gray-400 group-hover:text-gray-300 leading-relaxed font-light">
      ...
      <section className="py-20 md:py-32 bg-white">
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between mb-12 md:mb-20">
             ...
             <Link to="/projects" className="text-comfort-gold font-bold uppercase text-[10px] md:text-xs tracking-[0.2em] md:tracking-[0.3em] hover:text-comfort-blue transition-colors">Tout voir</Link>
          </div>

          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-8 md:grid-cols-3 gap-12">
            {projects.slice(0, 3).map((project) => (
              <div key={project.id} className="group">
                 <div className="aspect-video relative overflow-hidden mb-6 md:mb-8 shadow-xl">
                    <img src={project.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
                    <div className="absolute top-4 left-4 md:top-6 md:left-6 px-3 md:px-4 py-1.5 bg-white/90 backdrop-blur-md text-comfort-blue font-bold text-[9px] md:text-[10px] uppercase tracking-widest shadow-lg">
                      {project.status === 'Ongoing' ? 'En Cours' : 'Achevé'}
                    </div>
                 </div>
                 <div className="space-y-3 md:space-y-4">
                    <span className="text-comfort-gold font-bold text-[10px] md:text-xs uppercase tracking-widest">{project.category}</span>
      ...
      {/* 🏛️ SECTION 5: TESTIMONIALS */}
      {testimonials.length > 0 && (
      <section className="py-20 md:py-32 bg-comfort-dark text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
             <Heart className="mx-auto text-comfort-gold mb-8 md:mb-12 opacity-50 w-8 h-8 md:w-12 md:h-12" strokeWidth={1} />
             <div className="animate-in fade-in duration-1000">
                <blockquote className="text-xl md:text-4xl font-serif italic font-light leading-relaxed mb-8 md:mb-12 text-gray-200">
                  "{testimonials[currentTesti].content}"
                </blockquote>
                <div className="flex flex-col items-center">
                   <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border border-comfort-gold/30 p-1 mb-4 md:mb-6">
                   ...
                   <h5 className="font-bold text-white text-base md:text-lg tracking-widest uppercase">{testimonials[currentTesti].name}</h5>
                   <p className="text-comfort-gold text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] mt-2">{testimonials[currentTesti].role}</p>
                </div>
             </div>
          </div>
        </div>
      ...
      {/* 🏛️ SECTION 6: PARTNERS (Autoscrolling Marquee) */}
      {partners.length > 0 && (
      <section className="py-16 md:py-24 bg-white overflow-hidden relative">
        <div className="container mx-auto px-6 mb-8 md:mb-12 text-center">
           <span className="text-comfort-gold font-bold uppercase tracking-[0.4em] text-[10px]">Partenaires de confiance</span>
        </div>
        
        {/* Infinite Scroll Container */}
        <div className="relative flex overflow-hidden">
          <div className="flex animate-marquee space-x-12 md:space-x-24 items-center whitespace-nowrap py-4">
             {/* Doubling partners list for seamless loop */}
             {[...partners, ...partners, ...partners].map((p, i) => (
                <div key={`${p.id}-${i}`} className="flex-shrink-0 grayscale hover:grayscale-0 transition-all duration-500 px-4">
                  <img 
                    src={p.logo} 
                    alt={p.name} 
                    className="h-8 md:h-12 w-auto object-contain opacity-40 hover:opacity-100" 
                  />
                </div>
             ))}
          </div>
      ...
      {/* 🏛️ SECTION 7: CALL TO ACTION */}
      <section className="py-20 md:py-32 bg-white">
        <div className="container mx-auto px-6">
           <div className="bg-comfort-blue rounded-sm p-10 md:p-24 relative overflow-hidden">
              ...
              <div className="relative z-10 max-w-2xl">
                 ...
                 <Link to="/donate" className="inline-block bg-white text-comfort-blue px-8 md:px-12 py-4 md:py-5 font-bold uppercase tracking-widest hover:bg-comfort-gold hover:text-white transition-all shadow-2xl text-xs md:text-sm">
                    Agir maintenant
                 </Link>
              </div>
           </div>
